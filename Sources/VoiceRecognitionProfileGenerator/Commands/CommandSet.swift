class CommandSet {
  ...
  func childrenFor(command: Command) -> [Command] {
    childrenByParent[ObjectIdentifier(command)] ?? []
  }

  func hasChildren(_ command: Command) -> Bool {
    childrenByParent[ObjectIdentifier(command)] != nil
  }
  ...
  func add(command: Command, withAlias alias: String? = nil) throws {
    ...
    if let parent = command.parent {
      let parentId = ObjectIdentifier(parent)
      childrenByParent[parentId, default: []].append(command)
    }
